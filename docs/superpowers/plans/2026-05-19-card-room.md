...
- [ ] **Step 1: Verify auto-discovery (no changes needed)**

`index.js` uses `eagerLoadControllersFrom` which auto-discovers any `*_controller.js` file. No manual registration needed. Just confirm:

```bash
grep eagerLoadControllersFrom app/javascript/controllers/index.js
```
